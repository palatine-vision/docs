...
with open("audio.mp3", "rb") as f:
	result = client.audio.transcriptions.create(
        file=f,
        model="palatine_large_highspeed", # так же доступна более быстрая модель palatine_small
        # при необходимости можно выбрать один из вариантов форматов ответа
        # response_format="json" | "verbose_json" | "text" | "srt" | "vtt"
        # extra_query={"fast_inference": False} если необходимо выключить более быстрый движок для инференса с целью улучшения качества транскрибации (по-умолчанию True)
    )
...
<AccordionGroup>
  ...
  <Accordion title="Проверка статуса задачи">
    ...
    ```
      ...
      Подробнее можно посмотреть [тут](https://docs.speech.palatine.ru/api-reference/transcribe/transcribe-polling-api/task-status#response-data)
  ...
  <Accordion title="Выгрузка результатов в файл">
    ...
    Поддерживаемые форматы: `srt`, `vtt`, `txt`, `csv` и `xlsx` — укажите нужное значение в параметре `file_format`.
    ...
    Эндпоинт вернёт файл при статусе задачи `success`, 404 — если задача не найдена, и 409 — если она ещё в обработке.
    Так удобно получить итоговые субтитры или таблицу без дополнительной пост‑обработки.
