...
<AccordionGroup>
  <Accordion title="Подключение к WebSocket">
    ...
    **Формат аудио для отправки:**
    ...
    При использовании сжатых кодеков (opus, mp3 и др.) добавляется ~10-20ms на декодирование.
    </Tip>
...
<AccordionGroup>
  <Accordion title="Рекомендуемые форматы (минимальная задержка)">
    **Raw PCM форматы** — без накладных расходов на декодирование:
    ...
    Эти форматы обрабатываются напрямую декодирования, что обеспечивает минимальную задержку (~10-50ms).
  </Accordion>
