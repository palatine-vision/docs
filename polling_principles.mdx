...
Это значит, что клиент получает идентификатор задачи (`task_id`) и периодически опрашивает сервер для получения статуса выполнения и результата.
...
2. **Опрос статуса**
   Клиент с помощью `task_id` периодически выполняет GET-запрос на endpoint статуса задачи:
...
3. **(Необязательно) Завершение задачи вручную**
...
STATUS_URL = "https://api.palatine.ru/api/v1/cloud_ai/transcription/task_status/{task_id}"
TOKEN = "<YOUR_TOKEN>"
